...
while ($row = $query->fetch_assoc()) {
	...
	$ids=$pn['id'];
	$rmb=$row['rombel'];
	if(file_exists( $_SERVER{'DOCUMENT_ROOT'} . "/apinnew/images/siswa/".$pn['avatar'])){
		$avatarm=$pn['avatar'];
	}else{
	...
	$namasis=$pn['nama'];
	$output['data'][] = array(
		"<img alt='image' src='../images/siswa/".$avatarm."' class='rounded-circle' width='35' data-toggle='tooltip' title='".$pn['nama']."'> ".$pn['nama'],
		$pn['nis'],
		$pn['nisn'],
		$tgl,
		$pn['jk'],
